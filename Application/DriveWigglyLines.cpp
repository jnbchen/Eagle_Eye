    ...
    void execute () {
      try{
        while (true) {
          ...
          Velocity dv;
          dv.velocity = (abs(deltay)>2000 || vp.position.x<500 || vp.position.x>12000 ? 0 : 0.5);
          double sign = vp.orientation.in_between(Angle::deg_angle(-90),Angle::deg_angle(90)) ? -1 : +1;
...
namespace {
  ...
  static DerWeg::PluginBuilder<DerWeg::KogmoThread, DerWeg::DriveWigglyLines> application ("DriveWigglyLines");
