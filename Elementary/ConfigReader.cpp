...
using namespace std;
using DerWeg::ConfigReader;

// Schluessel-Wert-Paare:
struct DerWeg::ConfigReader::KVPair {
  string key;
  string value;
...
namespace {
  ...
  bool is_whitespace (const string& s) {
    unsigned int n=static_cast<int>(s.length());
  ...
  bool find_sorted (unsigned int& rv, const C& container, const T& element) {
    unsigned int right=static_cast<unsigned int>(container.size());
    unsigned int left=0;
  ...
  void extract_path_from_filename (std::string& path, std::string& filename, const std::string& line) {
    ...
    unsigned int i=static_cast<unsigned int>(line.length()-1);
    while (i>0)
      if (line[i]=='/')
        break;
      else
  ...
  string replace_all (const string& s1, const string& s2, const string& s3) { // durchsucht s1 sequentiell nach Vorkommen von s2 und ersetzt diese durch s3
    string res = s1;
    unsigned int pos = 0;
    while ((pos = static_cast<unsigned int>(res.find (s2, pos)))<res.length()) {
      res.replace (pos, s2.length(), s3);
      pos+=static_cast<unsigned int>(s3.length());
    }
     return res;
  }
...
bool ConfigReader::file_input (const char* baksuffix, const char* name, const std::vector<std::string>& repkeys, bool recursive, std::string& prefix) throw (std::bad_alloc) {
  ...
  if (!filesuccess) {
    if (verbosity>=1) {
      EOUT("ConfigReader Error: cannot handle file " << filename << '\n');
    }
    directory_stack.pop();
    return false;
  }
  if (verbosity>=2) {
    if (replace_mode) {
      EOUT("ConfigReader: copy from file " << npath << filename << baksuffix << " to " << npath << filename << '\n');
    } else {
      EOUT("ConfigReader: read from file " << npath << filename << '\n');
    }
  }
  ...
  while (!(src->eof())) {
    ...
    unsigned int p=static_cast<unsigned int>(oline.find (comment_char));
    unsigned int n=static_cast<unsigned int>(oline.length());
    string line = oline;
    string comment = "";
    if (p<n) {
    ...
    if (line[0]=='[') {
      ...
      p=static_cast<unsigned int>(line.find (']'));
      if (p<n) {
        if (p==1) {
          prefix = "";
          if (verbosity>=3) {
            EOUT("ConfigReader: read from empty block\n");
          }
        } else {
          prefix = line.substr (1,p-1)+string("::");
          if (verbosity>=3) {
            EOUT("ConfigReader: read from block " << line.substr (1,p-1) << '\n');
          }
        }
        if (replace_mode)
          (*dest) << oline << '\n';
        ...
        success=false;
        if (verbosity>=1) {
          EOUT("ConfigReader Error: invalid block definition " << line << '\n');
        }
        if (replace_mode)
          (*dest) << oline << '\n';
      }
    ...
    p=static_cast<unsigned int>(line.find (assign_char));
    if (p<n) {
      KVPair new_pair;
      ...
      unsigned int pos;
      if (verbosity==3) {
        EOUT("ConfigReader: read line " << new_pair.key << '\n');
      }
      ...
      if (find_sorted (pos, key_value_table, new_pair)) {
        if (replace_mode) {
          ...
          if (key_value_table[pos].value!=new_pair.value && (replace_all_keys || (find(repkeys.begin(), repkeys.end(), new_pair.key)!=repkeys.end()))) {
            ...
            if (verbosity>=1) {
              EOUT("ConfigReader: replace line " << new_pair.key << " | " << new_pair.value << '\n');
            }
          } else {
            (*dest) << oline << '\n';
            if (verbosity>=3) {
              EOUT("ConfigReader: keep line " << new_pair.key << " | " << new_pair.value << '\n');
            }
          }
        } else {
          ...
          if (verbosity>=4) {
            EOUT("ConfigReader: replace value " << new_pair.key << " | " << new_pair.value << '\n');
          }
        }
      } else {
        ...
        if (replace_mode) {
          ...
          if (verbosity>=4) {
            EOUT("ConfigReader: insert value " << new_pair.key << " | " << new_pair.value << '\n');
          }
...
bool ConfigReader::find_first (string& value, const string& key) const {
  ...
  if (!found && verbosity>=1) {
    EOUT("ConfigReader Error: key " << key << " not found\n");
  }
  return found;
}
...
bool ConfigReader::find_all (deque<string>& values, const string& key) const {
  ...
  bool found = find_sorted (p, key_value_table, kv);
  if (found) {
    unsigned int n = static_cast<unsigned int>(key_value_table [p].value.length());
    string word;
    unsigned int q=0;
    while (q<n) {
...
void ConfigReader::append_from_command_line (unsigned int argc, char** argv) {
  const char** cargv = new const char* [argc];
  for (unsigned int i=0; i<argc; i++)
    cargv[i]=argv[i];
  append_from_command_line (argc, cargv);
  delete [] cargv;
}

void ConfigReader::append_from_command_line (unsigned int argc, const char** argv) {
  insert_pair (string("ConfigReader::program_name"), string(argv[0]));
  unsigned int index=1;
  unsigned int unknown_parameter_counter=0;
  while (index<argc) {
    ...
    if (argv[index][0]=='-') {
      if (argv[index][1]=='-') {
        unsigned int p=static_cast<unsigned int>(arg.substr(2,arg.length()-2).find (assign_char));
        if (p<arg.length()) {
          key = arg.substr(2,p);
          value = arg.substr(3+p, arg.length());
        } else {
          key = arg.substr(2,p);
          value = "1";
        }
        ...
        unsigned int longest_len_index=0;
        for (unsigned int i=0; i<shortcuts.size(); i++) {
          if (shortcuts[i].short_key.length()>longest_len && arg.substr(1,shortcuts[i].short_key.length())==shortcuts[i].short_key) {
            longest_len=static_cast<unsigned int>(shortcuts[i].short_key.length());
            longest_len_index=i;
          }
        }
        if (longest_len==0) {
          // kein shortcut gefunden, einfach uebernehmen
          key = arg.substr(1, arg.length()-1);
          value="1";
        } else {
          key = shortcuts[longest_len_index].long_key;
          if (shortcuts[longest_len_index].has_arg) {
            value = arg.substr(1+shortcuts[longest_len_index].short_key.length(), arg.length());
            if (value.length()==0 && index+1<argc && argv[index+1][0]!='-') {
              value=argv[index+1];
              index++;
            }
          } else {
      ...
      stringstream inout;
      inout << ++unknown_parameter_counter << '\n';
      std::getline (inout, key);
      key="ConfigReader::unknown_argument_"+key;
      value=argv[index];
    }
...
void ConfigReader::insert_pair (const std::string& k, const std::string& v) throw (std::bad_alloc) {
  ...
  if (find_sorted (pos, key_value_table, new_pair)) {
    ...
    // neuen Eintrag erzeugen
    if (verbosity>=4) {
      EOUT("ConfigReader: insert line " << new_pair.key << " | " << new_pair.value << '\n');
    }
    key_value_table.insert (key_value_table.begin()+pos, new_pair);
  }
}
