...
unsigned int MultiPacketUDPCommunication::send (const std::vector<char*> buffers, const std::vector<unsigned int> buflens) throw () {
  ...
  unsigned int num_packets = static_cast<unsigned int>(buffers.size());
  if (buflens.size()<buffers.size())
    num_packets=static_cast<unsigned int>(buflens.size());
    ...
    if (success) {
      ...
      for (unsigned int i=0; i<senderAddresses.size(); i++)
        ...
        if (sender_index==-1) {
          ...
          sender_index = static_cast<unsigned int>(senderAddresses.size())-1;
        }
