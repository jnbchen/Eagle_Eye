...
/// Default constructor
///--------------------------------------------------------------------------------------///
LandmarkFinder::LandmarkFinder(const DerWeg::ConfigReader& cfg, CalibIO& calibration,std::string cfgfile_darkframe)
{
    CvMat* pCu = nullptr;
    CvMat* pCv = nullptr;
    calibration.computeLUT(0,0, pCu, pCv);

    m_oCalibMap_u = cv::Mat(pCu).clone();
    m_oCalibMap_v = cv::Mat(pCv).clone();


    /// get stuck pixel
    cv::Mat oDarkFrame;
    oDarkFrame = cv::imread(cfgfile_darkframe.c_str(), 0);
    /// identify all stuck pixels: go thru all pixels and compare them to theshold
    unsigned char cStuckPixelThresh = 2;

    for(int nY = 0; nY < oDarkFrame.rows; nY++)
    {
        for(int nX = 0; nX < oDarkFrame.cols; nX++)
        {
            if(cStuckPixelThresh <= oDarkFrame.at<unsigned char>(nY,nX))
            {
                m_vStuckPixels.push_back(cv::Point(nX,nY));
                //std::cout << "Stuck pixel at " << nX << "," << nY << std::endl;
            }
        }
    }

    std::vector<cv::Point>::iterator pStuckPixel;
    for(pStuckPixel = m_vStuckPixels.begin(); pStuckPixel != m_vStuckPixels.end(); pStuckPixel++)
    {
        cv::circle(oDarkFrame, cv::Point(pStuckPixel->x, pStuckPixel->y), 5, cv::Scalar(255), 2);
    }

    //cv::namedWindow("Stuck Pixels");
    //cv::imshow("Stuck Pixels", oDarkFrame);
    ...
    m_fMaxRadiusForCluster      = 50;
    m_nMinPointsPerLandmark     = 5;
    m_nMaxPointsPerLandmark     = 9;

    /// Read in Landmark ids
    if (!cfg.get ("CameraLocalization::landmarks", m_vnIDs)) {
...
void LandmarkFinder::SetImage(cv::Mat& i_oImage)
{
    ...
    i_oImage.assignTo(Image3C, CV_8UC3);
        ...
        /// mask out stuck pixels
    //int nNewX, nNewY;
//    std::vector<cv::Point>::iterator pStuckPixel;
//    for(pStuckPixel = m_vStuckPixels.begin(); pStuckPixel != m_vStuckPixels.end(); pStuckPixel++)
//    {
        /// set this pixel to zero
//        Image3C.at<cv::Vec3b>(pStuckPixel->y, pStuckPixel->x) = cv::Vec3b(0,0,0);

//        /// sum up neighbouring values
//        for(int nX = -1; nX <= 1; nX++)
//        {
//            nNewX = pStuckPixel->x + nX;
//            for(int nY = -1; nY <= 1; nY++)
//            {
//                nNewY = pStuckPixel->y + nY;
//                if((nX != 0 || nY != 0)
//                   && (nNewX >= 0 && nNewY >= 0)
//                   && (nNewX < i_oImage.cols && nNewY < i_oImage.rows))
//               {
//                    i_oImage.at<cv::Vec3b>(pStuckPixel->y, pStuckPixel->x) += i_oImage.at<cv::Vec3b>(nNewY, nNewX);
//               }
//            }
//        }
//        /// average
//        Image3C.at<cv::Vec3b>(pStuckPixel->y, pStuckPixel->x) *= 1.0/8.0;
//    }
//    cv::namedWindow("Orig",CV_WINDOW_NORMAL);
//    cv::imshow("Orig", Image3C);
    //cv::waitKey(10);
    cv::remap(Image3C,m_oImage,m_oCalibMap_u,m_oCalibMap_v,cv::INTER_LINEAR);

    //std::cout << m_oImage.rows << "x" << m_oImage.cols << std::endl;

    //cv::imwrite("/home/anicar/Desktop/KognitiveAutomobileLabor/trunk/bin/test.png", m_oImage);
}
...
///--------------------------------------------------------------------------------------///
int LandmarkFinder::FindLandmarks(std::vector<Landmark>& o_vLandmarks)
{
    ...
    std::vector<Landmark>::iterator pLandmark;
    cv::Point Test(0,0);
    ...
    cv::cvtColor( m_oImage, m_oGrayImage, CV_BGR2GRAY );
    ...
    FindClusters(ClusteredPixels, ClusteredPoints, m_fMaxRadiusForCluster, m_nMinPointsPerLandmark, m_nMaxPointsPerLandmark);


    /// on the clustered points, extract corners
    /// output is of type landmark, because now you can almost be certain that what you have is a landmark
    o_vLandmarks = FindCorners(ClusteredPoints);
    //LOUT ("FINDLAND: " << ClusteredPixels.size() << ' ' << o_vLandmarks.size() << ' ' << std::endl);

    GetIDs(o_vLandmarks);

    if (false)   /// set to true for nice visualisation
    {
        for(pClusteredPoints = ClusteredPoints.begin(); pClusteredPoints != ClusteredPoints.end(); pClusteredPoints++)
        {
            Test=cvPoint(0,0);
            for(pClusteredPixels = pClusteredPoints->begin(); pClusteredPixels != pClusteredPoints->end(); pClusteredPixels++)
            {
                Test += *pClusteredPixels;
            }
            Test = 1/float(pClusteredPoints->size())*Test;
            cv::circle(m_oImage, Test, m_fMaxRadiusForCluster, cv::Scalar(0,0,255), 2);//For Blue: Scalar(255, 0, 0)
        }

        for(pLandmark = o_vLandmarks.begin(); pLandmark != o_vLandmarks.end(); pLandmark++)
        {
            cv::circle(m_oImage, pLandmark->voCorners.at(0), 4, cv::Scalar(0,255,0), 3);
            cv::circle(m_oImage, pLandmark->voCorners.at(1), 4, cv::Scalar(0,255,0), 2);
            cv::circle(m_oImage, pLandmark->voCorners.at(2), 4, cv::Scalar(0,255,0), 3);
            //
            out1<<pLandmark->nID;
            sID = out1.str();
            putText(m_oImage, sID, cvPoint(pLandmark->oPosition.x+m_fMaxRadiusForCluster,pLandmark->oPosition.y+m_fMaxRadiusForCluster), 2, 0.4 ,cvScalar(0,0,255) );
            out1.str(std::string());
        }

        /// from the clusters with corner points, try to find a valid ID to match every landmark
        /// this only works on Landmark clusters

        cv::namedWindow("Landmark Image",CV_WINDOW_NORMAL);

        for(pClusteredPixels = ClusteredPixels.begin(); pClusteredPixels != ClusteredPixels.end(); pClusteredPixels++)
        {
            cv::circle(m_oImage, *pClusteredPixels, 2, cv::Scalar(255,0,0), 2);//For Blue: Scalar(255, 0, 0)
        }
...
///
///--------------------------------------------------------------------------------------///
void LandmarkFinder::Check(cv::Mat& Filtered, int XPos, int YPos, int Threshold, int& Pixelcount, int& SummedX, int& SummedY)
{   int x, y;
...
//cv::imwrite("/home/anicar/Desktop/KognitiveAutomobileLabor/trunk/bin/filtered.png", filtered);
...
// alternative 1:  without region growing
    ...
    for(pAllClustersIt = ClusteredPixels.begin(); pAllClustersIt != ClusteredPixels.end(); pAllClustersIt++)
    {
        ...
        for(pThisClusterIt = pAllClustersIt->begin(); pThisClusterIt != pAllClustersIt->end(); pThisClusterIt++)
        {/// go thru all points in this cluster
...
///
///--------------------------------------------------------------------------------------///
void LandmarkFinder::FindClusters(std::vector<cv::Point>& i_voPoints, std::vector<Cluster>& o_voCluster, const float i_fRadiusThreshold, const unsigned int i_nMinPointsThreshold, const unsigned int i_nMaxPointsThreshold)
{
    ...
    for(pPointsIt = i_voPoints.begin(); pPointsIt != i_voPoints.end(); pPointsIt++)  /// go thru all points
    {
        ...
        for(pAllClustersIt = o_voCluster.begin(); pAllClustersIt != o_voCluster.end(); pAllClustersIt++)
        {/// go thru all clusters
            for(pThisClusterIt = pAllClustersIt->begin(); pThisClusterIt != pAllClustersIt->end(); pThisClusterIt++)
            {/// go thru all points in this cluster
                /// calculate distance to a point
                Dist = *pThisClusterIt-ThisPoint;
                if(i_fRadiusThreshold >= sqrt((float)(Dist.x*Dist.x + Dist.y*Dist.y))) /// if distance is smaller than threshold, add point to cluster
                {
            ...
            if(cUsed)
            {/// because point has been added to cluster, no further search is neccessary
                break;
            }
        }
    ...
    {
        if(i_nMinPointsThreshold > pAllClustersIt->size() || i_nMaxPointsThreshold < pAllClustersIt->size())
        { /// if there are too few or too many points within this cluster, delete it
...
///--------------------------------------------------------------------------------------///
/// FindCorners identifies the landmark corners and puts them into the landmark structure
///
///--------------------------------------------------------------------------------------///
std::vector<Landmark> LandmarkFinder::FindCorners(std::vector<Cluster>& ClusteredPoints)
{
    ...
    std::vector<Landmark> OutputLandmarks;

    bool bCornersSet;

    for(pAllClustersIt = ClusteredPoints.begin(); pAllClustersIt != ClusteredPoints.end(); pAllClustersIt++)
    { /// go thru all clusters
        bCornersSet = false;

        /// in each cluster, find three points that sum of length is maximum and two edges are perpendicular
        fMaxfunc = -10000;
        ...
        ThisLandmark.nErrors = 0;   /// we have no detection so far, so error count in detections is zero
        ThisLandmark.nID = 0;       /// we have not identified anything, so default ID is zero


        ThisLandmark.voIDPoints = *pAllClustersIt; /// all points in this cluster are copied to the ID point vector for further examination

        ThisLandmark.nPointCount = ThisLandmark.voIDPoints.size(); /// the total number of available points in this landmark, this might be of interest if misdetections happen



        /// now, go thru all points and compare them with all the other points
        for(pPointIterator = ThisLandmark.voIDPoints.begin(); pPointIterator != ThisLandmark.voIDPoints.end(); pPointIterator++)
        {

            ThisPoint = *pPointIterator;

            /// since a point has been compared to all points befor it, we start further comparison here
            for(pAdjecentPointIt = pPointIterator; pAdjecentPointIt != ThisLandmark.voIDPoints.end(); pAdjecentPointIt++)
            {
                ...
                if(pAdjecentPointIt != pPointIterator)
                {
                    ...
                    for(pAdjecentPointIt2 = pAdjecentPointIt; pAdjecentPointIt2 != ThisLandmark.voIDPoints.end(); pAdjecentPointIt2++)
                    {
                        /// if the third point under examination is equal to the first or second, dont do anything
                        if((pAdjecentPointIt2 != pPointIterator) && (pAdjecentPointIt2 != pAdjecentPointIt))
                        {
                            ...
                            if (((Dist2.x*Dist2.x+Dist2.y*Dist2.y)>fp*(Dist3.x*Dist3.x+Dist3.y*Dist3.y)) && ((Dist2.x*Dist2.x+Dist2.y*Dist2.y)>fp*(Dist.x*Dist.x+Dist.y*Dist.y)))
                                {
                                Dist = *pPointIterator-*pAdjecentPointIt2;
                                Dist2 = *pPointIterator-*pAdjecentPointIt;
                                Dist3 = *pAdjecentPointIt-*pAdjecentPointIt2;
                                }
                            else if (((Dist3.x*Dist3.x+Dist3.y*Dist3.y)>fp*(Dist2.x*Dist2.x+Dist2.y*Dist2.y)) && ((Dist3.x*Dist3.x+Dist3.y*Dist3.y)>fp*(Dist.x*Dist.x+Dist.y*Dist.y)))
                                {
                         ...
                         //4   fProjection = -exp(-fabs(asin((Dist3.x*Dist2.x+Dist3.y*Dist2.y)/fDist2/fDist3)))*50;
                         //3   fProjection = fabs((Dist3.x*Dist2.x+Dist3.y*Dist2.y))/fDist2/fDist3*50;
                         //2   fProjection = fabs(asin((Dist3.x*Dist2.x+Dist3.y*Dist2.y)/fDist2/fDist3))*50;
                            fProjection = fabs((Dist3.x*Dist2.x+Dist3.y*Dist2.y))/fDist2/fDist3;//+fabs((Dist2.x*Dist3.x+Dist2.y*Dist3.y))/fDist3/fDist3;
                            fDiffOfLength = fabs(fDist2-fDist3);

                            //LOUT("Sum of length " << fSumOfLength << " Projection " << fProjection << " Diff " << fDiffOfLength << std::endl);
        ...
        /// note the reversed order: it's 1-3-2, because the corner 3 is the one between 1 and 2
        /// this helps for post-processing
        if(((Dist.x*Dist.x+Dist.y*Dist.y)>fp*(Dist2.x*Dist2.x+Dist2.y*Dist2.y)) && ((Dist.x*Dist.x+Dist.y*Dist.y)>fp*(Dist3.x*Dist3.x+Dist3.y*Dist3.y)))
            {
        ...
        else if (((Dist2.x*Dist2.x+Dist2.y*Dist2.y)>fp*(Dist3.x*Dist3.x+Dist3.y*Dist3.y)) && ((Dist2.x*Dist2.x+Dist2.y*Dist2.y)>fp*(Dist.x*Dist.x+Dist.y*Dist.y)))
            {
        ...
        ThisLandmark.voIDPoints.erase(pCornerOne);
            ...
            /// the marker is defined as the middel of the connection between the two outer corners
...
///--------------------------------------------------------------------------------------///
int LandmarkFinder::GetIDs(std::vector<Landmark>& io_voLandmarks)
{
    /*  Numbering of corners and coordinate frame
     *       ---> y
     *  |   1   .   .   .
    ...
    /// once we've been through all landmarks, we can look up available IDs in the vector define above.
    /// this is why we remember errors but don't correct them right away.
    std::vector<Landmark> voLandmarksInQueue;

    /// go thru all landmarks
    std::vector<Landmark>::iterator pLandmarkIt = io_voLandmarks.begin();
    while(pLandmarkIt != io_voLandmarks.end())
    {
        ...
        /// now, we find the affine transformation which maps the landmark from image coordinate in a landmark-related coordinate frame
        ...
        /// now we have a transform which maps [0,1028]x[0,1280] -> [0,1]x[0,1], i.e. our landmark is in the latter domain

        /// next, the ID points are transformed accordingly and then matched to their binary values
        ...
        {
            /// first step: bring the ID point in relation to the origin of the landmark
            ...
            /// it's 1-y because in the definition of the landmark ID the x axis runs down
            int nY = floor((y)/0.25);
        ...
        */
        ...
        {
            if(*pIDLUTIt == ID)
            { /// ID matches one which is available: stop search
                break;
            }
        }

        if(pIDLUTIt != vnIDs.end())
        { /// ID matches one which is available:
            vnIDs.erase(pIDLUTIt); /// remove this ID
            ++pLandmarkIt; /// go to next landmark
        }
        else
        { /// no ID match
            voLandmarksInQueue.push_back(*pLandmarkIt); /// put this landmark in queue for second processing run
            io_voLandmarks.erase(pLandmarkIt); /// delete it from valid landmark list. This also is a step to next landmark
        }
    }
    ...
    std::vector<Landmark>::iterator pvoLandmarksInQueue;
    for(pvoLandmarksInQueue = voLandmarksInQueue.begin(); pvoLandmarksInQueue != voLandmarksInQueue.end(); pvoLandmarksInQueue++)
    {
        nThisID = 0;

        /// same as before: finde affine transformation, but this time from landmark coordinates to image coordinates
        ...
        /// go thru all possible ID points and see if the image has a high gray value there, i.e. there's light
        for(int nX = 0; nX < 4; nX++)
        {
            for(int nY = 0; nY < 4; nY++)
            {
                ...
                {
                    int ThisPointID = 0;
                    /// since we know the corners, we can go in thirds between them to see if theres a light
                    ...
                    /// same as for the pixel detection: see if the gray value at the point where the light should be exceeds a threshold and thus supports the light hypothesis
                    if(0 > Index.x || 0 > Index.y || m_oGrayImage.cols <= Index.x || m_oGrayImage.rows <= Index.y)
                    {
                        continue;
                    }

                    if(m_cThreshold < m_oGrayImage.at<unsigned char>(Index.y, Index.x))
