...
void plotAll (const LandmarkList& landmarks, const RobotPositionList& positions, const MeasurementList& measurements, bool ignore_isolated, CameraLocalizationMapping& cal) {
  ...
  for (unsigned int il=0; il<landmarks.size(); ++il) {
    tmpfile1 << landmarks[il].position.x << ' ' << landmarks[il].position.y << '\n';
  }
  tmpfile1 << '\n';
    ...
    while (true) {
      ...
      switch (c) {
        ...
        case 'm' : {
          ...
          cout << "average error=" << cal.totalPositionError (1.0) << endl;
          break;
        }
        case 'M' : {
          unsigned int nmax;
          cin >> nmax;
          cal.calibrateLandmarks (nmax);
          cout << "average error=" << cal.totalPositionError (1.0) << endl;
          break;
        }
        case 'e' : cout << "average error=" << cal.totalPositionError (1.0) << endl; break;
        case 'd' : {
          double maxstd;
          cin >> maxstd;
        ...
        case 'v' : {
          ...
          cin >> filename;
          try{
            CameraLocalizationMapping valid;
            ConfigReader cfg;
            ...
            valid.determinePositions (true);
            cout << "average error=" << valid.totalPositionError (1.0) << endl;
        ...
        default : {
          ...
          cout << "m: recreate map\n";
          cout << "M <nmax>: recreate map with nmax iterations\n";
